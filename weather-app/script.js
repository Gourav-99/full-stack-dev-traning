...
const key = `9U6A856DJLLJD8HXCG7G448TU`;
let url, locName;
const weatherCard = (res) => {
  const currentCond = res.currentConditions;
  const data = `
  <div class="flex justify-center">
      ...
      <h2 class="text-md mb-2 px-4 pt-4">
        <div class="flex justify-between">
          <div class="badge relative top-0">
            <span class="mt-2 py-1 h-12px text-md font-semibold w-12px  rounded right-1 bottom-1 px-4">${locName}</span></div>
          <span class="text-lg font-bold ">${new Date().toLocaleTimeString()}</span>
        </div>
      </h2>
    ...
    currentCond.humidity
  }%</span></span>
          </div>
          <div>
            <h1 class="text-6xl"> ${currentCond.temp}° </h1>
          </div>
        </div>
      </div>
    </div>
...
const getApiByName = (loc) => {
  url = `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${loc}?unitGroup=metric&include=current&key=${key}&contentType=json`;
};
const getApiByCords = (lat, long) => {
  url = `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${lat},${long}?unitGroup=metric&include=current&include=locationNames&key=${key}`;
};
...
const getWeatherByName = async (location) => {
  await getApiByName(location);
  await fetchWeather();
};
const getWeatherByCords = async (lat, long) => {
  await getApiByCords(lat, long);
  await fetchWeather();
};
const getLocName = async (lat, long) => {
  try {
    const locRes = await fetch(
      `https://api.api-ninjas.com/v1/reversegeocoding?lat=${lat}&lon=${long}`,
      {
        headers: {
          "X-Api-Key": "YqCcaC6TMMNC8GlXeKoDNw==DAEDIasDLAabcbLR",
        },
      }
    );
    const locData = await locRes.json();
    console.log(locData[0].name);
    locName = locData[0].name;
    input.value = locName;
    await getWeatherByCords(lat, long);
  } catch (error) {
    alert(error);
    console.log(error);
  }
  ...
  // console.log(locRes);
  // getApiByName(locData[0].name);
  // getWeatherByName(locData[0].name);
};
...
detectLoc.addEventListener("click", (e) => {
  ...
  navigator.geolocation.getCurrentPosition((position) => {
    getLocName(position.coords.latitude, position.coords.longitude);
  });
});
